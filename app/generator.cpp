...
void run(const arguments &args, size_t seed, const std::string& output) {
    LOG << "";
    LOG << "R-MAT generation using " << rmat_t::name << " method";

    RNG gen(seed);
    rmat_t r(gen, args.log_nodes, args.par_a, args.par_b, args.par_c, output);
...
int main(int argc, char *argv[]) {
    ...
    std::string output;
    clp.add_string('o', "output", output, "output file");
    ...
    if (scramble)
        run<RNG, rmat::rmat<true>>(args, seed, output);
    else
        run<RNG, rmat::rmat<false>>(args, seed, output);
